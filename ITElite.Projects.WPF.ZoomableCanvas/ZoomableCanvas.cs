//#error Thanks for downloading this file from Kael Rowan's blog (http://blogs.msdn.com/kaelr)
//#error Please know that it won't compile without the following code:
//#error VirtualPanel: http://blogs.msdn.com/b/kaelr/archive/2009/04/27/virtualpanel.aspx
//#error PriorityQuadTree: http://blogs.msdn.com/b/kaelr/archive/2009/05/21/priorityquadtree.aspx
//#error MathExtensions: http://blogs.msdn.com/b/kaelr/archive/2008/05/12/mathextensions.aspx
...
{
    /// <summary>
    ///     Defines an area within which you can explicitly position an infinite number of child elements by using coordinates
    ///     that are relative to the <see cref="ZoomableCanvas" /> area.
    /// </summary>
    public class ZoomableCanvas : VirtualPanel, IScrollInfo
    {
        ...
        /// </summary>
        public static readonly DependencyProperty ApplyTransformProperty = DependencyProperty.Register(
            "ApplyTransform", typeof (bool), typeof (ZoomableCanvas),
        ...
        /// <remarks>
        ///     The value of this dependency property is <c>true</c> by default, meaning that the <see cref="RenderTransform" />
        ///     property will contain a <see cref="Transform" /> that scales the canvas and its children automatically.
        ///     This property can be set to <c>false</c> prevent the automatic transform.  This means that children are responsible
        ///     for changing their appearance when the <see cref="Scale" /> property changes.
        ///     Note that this property does not affect the <b>placement</b> of the elements; the children are automatically placed
        ///     with the top-left corners of their elements at the appropriate positions on the screen, regardless of the value of
        ///     <see cref="ApplyTransform" />.
        ///     <para>
        ///         Children will usually do this by simply changing their <see cref="Width" /> and <see cref="Height " /> to
        ///         become larger or smaller when the <see cref="Scale" /> property increases or decreases.
        ///         This is useful when pen widths are important, such as an element surrounded with a <see cref="Border" /> with
        ///         <see cref="Border.BorderThickness" /> set to <c>1.0</c>.
        ///         If <see cref="ApplyTransform" /> is <c>true</c>, then as <see cref="Scale" /> decreases the shape will be
        ///         scaled down and the border stroke will become thinner than one pixel, possibly too thin to see even with
        ///         sub-pixel rendering.
        ///         This is also true when drawing paths, edges of a graph, or any other element that uses <see cref="Pen" /> to
        ///         draw lines and strokes.
        ///         In these cases setting <see cref="ApplyTransform" /> to <c>false</c> and setting the <see cref="Shape" />'s
        ///         <see cref="Shape.Stretch" /> to <see cref="Stretch.Fill" /> while binding its <see cref="Width" /> and
        ///         <see cref="Height" /> to a factor of <see cref="Scale" /> will often provide a better effect.
        ///     </para>
        ///     <para>
        ///         Another reason to set this property to <c>false</c> is when elements change their representation or visual
        ///         state based on the scale (also known as "semantic zoom").
        ///         For example, imagine a canvas showing multiple thumbnails of spreadsheets and the relationships between their
        ///         formulas and values.
        ///         When <see cref="Scale" /> is set to <c>1.0</c> (the default value), each spreadsheet element might be fully
        ///         interactive, editable, and showing all rows and columns.
        ///         When zooming out, and <see cref="Scale" /> gets small enough that there is not enough room for each spreadsheet
        ///         to show all of its rows and columns, it may change its representation into a bar chart or pie chart with axis
        ///         values and a legend instead.
        ///         When zooming even further out, and <see cref="Scale" /> gets small enough that there is not enough room for the
        ///         axis and legend, it may simply remove the axis and legend to make more room for the graphical portion of the
        ///         chart.
        ///         Since the children of the canvas can be arbitary rich UIElements, they can dynamically change their
        ///         representation and be interacted with at all levels of zoom.
        ///         This is in sharp contrast to multi-scale-image approaches such as Silverlight's Deep Zoom since those scenarios
        ///         are simply performing linear scale transformations on pre-computed static bitmaps.
        ///     </para>
        /// </remarks>
        ...
        /// <summary>
        ///     Returns a transform applying the <see cref="Scale" /> and <see cref="Offset" /> when <see cref="ApplyTransform" />
        ///     is set to <c>true</c>.
        /// </summary>
        ...
        public static readonly DependencyProperty ActualViewboxProperty =
            DependencyProperty.RegisterReadOnly("ActualViewbox", typeof (Rect), typeof (ZoomableCanvas),
                new FrameworkPropertyMetadata(Rect.Empty, OnActualViewboxChanged, CoerceActualViewbox))
                .DependencyProperty;
        ...
        /// <remarks>
        ///     The value of this property is automatically computed based on the <see cref="Scale" />, <see cref="Offset" />, and
        ///     <see cref="RenderSize" /> of this panel.
        ...
        /// <param name="value">The original uncoerced value.</param>
        /// <returns>
        ///     A <see cref="Rect" /> representing the area of the canvas (in canvas coordinates) that is being displayed by
        ///     this panel.
        /// </returns>
        ...
        /// </summary>
        public static readonly DependencyProperty ViewboxProperty = DependencyProperty.Register("Viewbox", typeof (Rect),
            typeof (ZoomableCanvas),
            new FrameworkPropertyMetadata(Rect.Empty, FrameworkPropertyMetadataOptions.BindsTwoWayByDefault,
                OnViewboxChanged), IsViewboxValid);

        /// <summary>
        ///     Gets or sets the portion of the canvas (in canvas coordinates) that should be attempted to be displayed by this
        ///     panel.
        /// </summary>
        /// <value>
        ///     A <see cref="Rect" /> specifying the portion of the canvas that should be displayed by this panel, or
        ///     <see cref="Rect.Empty" /> when unspecified.  The default value is <see cref="Rect.Empty" />.
        /// </value>
        /// <remarks>
        ///     The area of the canvas shown by this panel can be controlled by either setting <see cref="Scale" /> and
        ///     <see cref="Offset" />, or by setting the <see cref="Viewbox" />, <see cref="Stretch" />, and
        ///     <see cref="StretchDirection" /> properties.
        ///     When <see cref="Viewbox" /> is set to anything other than <see cref="Rect.Empty" />, the <see cref="Scale" /> and
        ///     <see cref="Offset" /> will be automatically coerced to appropriate values according to the <see cref="Stretch" />
        ///     and <see cref="StretchDirection" /> properties.
        ///     Note that the <see cref="Stretch" /> mode of <see cref="Stretch.Fill" /> is not supported, so unless the aspect
        ///     ratio of <see cref="Viewbox" /> exactly matches the aspect ratio of <see cref="RenderSize" /> the actual area
        ///     displayed will be more or less than <see cref="Viewbox" />.
        ...
        public static readonly DependencyProperty StretchProperty = DependencyProperty.Register("Stretch",
            typeof (Stretch), typeof (ZoomableCanvas), new FrameworkPropertyMetadata(Stretch.Uniform, OnStretchChanged),
            IsStretchValid);
        ...
        /// </summary>
        /// <value>
        ///     One of the <see cref="Stretch" /> values other than <see cref="Stretch.Fill" />.  The default is
        ///     <see cref="Stretch.Uniform" />.
        /// </value>
        /// <remarks>
        ///     Please see the documentation of <see cref="TileBrush.Stretch" /> for a detailed explanation of the effects of this
        ///     property.
        ///     The <see cref="Stretch" /> mode of <see cref="Stretch.Fill" /> is not supported, so unless the aspect ratio of
        ///     <see cref="Viewbox" /> exactly matches the aspect ratio of <see cref="RenderSize" /> the actual area displayed will
        ///     be more or less than <see cref="Viewbox" />.
        ///     The exact area that is displayed can be determined by the <see cref="ActualViewbox" /> property in this case.
        /// </remarks>
        ...
        public static readonly DependencyProperty StretchDirectionProperty =
            DependencyProperty.Register("StretchDirection", typeof (StretchDirection), typeof (ZoomableCanvas),
        ...
        /// <remarks>
        ///     When setting the <see cref="Viewbox" /> property, the <see cref="Scale" /> and <see cref="Offset" /> properties are
        ///     automatically coerced to the appropriate values according to the <see cref="Stretch" /> and
        ///     <see cref="StretchDirection" /> properties, and any existing values of <see cref="Scale" /> and
        ///     <see cref="Offset" /> will be overridden.
        ///     However, when the value of <see cref="StretchDirection" /> is set to anything other than
        ///     <see cref="StretchDirection.Both" />, then the setting of the <see cref="Scale" /> property can limit the range of
        ...
        /// <summary>
        ///     Handles the event that occurs when the value of the <see cref="StretchDirection" /> dependency property has
        ///     changed.
        /// </summary>
        /// <param name="d">The dependency object on which the dependency property has changed.</param>
        /// <param name="e">The event args containing the old and new values of the dependency property.</param>
        ...
        /// </summary>
        public static readonly DependencyProperty OffsetProperty = DependencyProperty.Register("Offset", typeof (Point),
            typeof (ZoomableCanvas),
            new FrameworkPropertyMetadata(new Point(0.0, 0.0), FrameworkPropertyMetadataOptions.BindsTwoWayByDefault,
                OnOffsetChanged, CoerceOffset), IsOffsetValid);

        /// <summary>
        ///     Gets or sets (but see remarks) the top-left point of the area of the canvas (in canvas coordinates) that is being
        ...
        ///     <para>
        ///         For example, consider a child element which has its <see cref="Canvas.Left" /> and <see cref="Canvas.Top" />
        ///         set to <c>100</c> and <c>100</c>.
        ///         Also assume that the value of <see cref="Scale" /> is set to <c>1.0</c> (the default value).
        ///     </para>
        ///     <para>
        ///         By default, the value of <see cref="Offset" /> is <c>(0,0)</c> so the element will be displayed at 100 units to
        ///         the right and 100 units down from the top-left corner of this panel, exactly how <see cref="Canvas" /> would
        ///         display it.
        ///         If the value of <see cref="Offset" /> is set to <c>(20,40)</c> then the element will be displayed 80 units to
        ///         the right and 60 units down from the top-left corner of this panel.
        ///         In other words, it will have appeared to "move" up by 20 units and left by 40 units.
        ///     </para>
        ///     <para>
        ///         If the value of <see cref="Offset" /> is set to <c>(100,100)</c> then the top-left corner of the element will
        ///         be displayed exactly in the top-left corner of this panel.
        ///         Note that this is true regardless of the value of <see cref="Scale" />!
        ///     </para>
        ///     <para>
        ///         If the value of <see cref="Offset" /> is set to <c>(110,120)</c> then the top-left corner of the element will
        ///         be displayed 10 units to the left and 20 units above this panel.
        ///         In other words, if <see cref="ClipToBounds" /> is set to <c>true</c>, then the top-left corner of the element
        ///         will not be visible.
        ///     </para>
        ///     <para>
        ///         The value of <see cref="Offset" /> can also be negative, so if the value of <see cref="Offset" /> is set to
        ///         <c>(-100,-100)</c> then the element will be displayed at 200 pixels to the right and 200 pixels down from the
        ///         top-left corner of the panel.
        ///     </para>
        ///     <para>
        ///         When the <see cref="Viewbox" /> property is set to a non-<see cref="Rect.Empty" /> value, the value of the
        ///         <see cref="Offset" /> property will be automatically computed to match the <see cref="Viewbox" />,
        ///         <see cref="Stretch" />, and <see cref="StretchDirection" /> properties.
        ///         The value of the <see cref="Offset" /> property will contain the computed value (via the WPF dependency
        ///         property coersion mechanism), and any attempts to set <see cref="Offset" /> to a different value will be
        ///         ignored until <see cref="Viewbox" /> is set to <see cref="Rect.Empty" /> again.
        ///     </para>
        /// </remarks>
        /// <seealso cref="Viewbox" />
        public Point Offset
        ...
        /// <summary>
        ///     Returns a <see cref="Point" /> representing top-left point of the canvas (in canvas coordinates) that is currently
        ///     being displayed after taking <see cref="Viewbox" /> into account.
        /// </summary>
        /// <param name="d">Dependency object whos value is being coerced.</param>
        /// <param name="value">The original uncoerced value.</param>
        /// <returns>
        ///     A <see cref="Point" /> representing top-left point of the canvas (in canvas coordinates) that is currently
        ///     being displayed.
        /// </returns>
        ...
        public static readonly DependencyProperty ScaleProperty = DependencyProperty.Register("Scale", typeof (double),
            ...
            new FrameworkPropertyMetadata(1.0, FrameworkPropertyMetadataOptions.BindsTwoWayByDefault, OnScaleChanged,
                CoerceScale), IsScaleValid);

        /// <summary>
        ///     Gets or sets (but see remarks) the scale at which the content of the canvas is being displayed.
        /// </summary>
        /// <value>
        ///     A <see cref="Double" /> between <see cref="Double.Epsilon" /> and <see cref="Double.MaxValue" />.  The default
        ///     value is <c>1.0</c>.
        /// </value>
        /// <remarks>
        ///     This value is what controls the zoom level of the canvas and the amount of the <see cref="ScaleTransform" /> when
        ///     <see cref="ApplyTransform" /> is set to <c>true</c>.
        ///     When <see cref="ApplyTransform" /> is set to <c>false</c>, this value still controls the positioning of the
        ///     children (i.e. elements are placed closer together when zoomed out and farther apart when zoomed in), but the sizes
        ///     of the children are unaffected.
        ///     <para>
        ///         For example, consider a child element which has its <see cref="Canvas.Left" /> and <see cref="Canvas.Top" />
        ///         set to <c>100</c> and <c>100</c>, with a <see cref="Width" /> of 50 and a <see cref="Height" /> of 50.
        ///         Also assume that the value of <see cref="Offset" /> is set to <c>(0,0)</c> (the default value).
        ///     </para>
        ///     <para>
        ///         By default, the value of <see cref="Scale" /> is <c>1.0</c> so the element will be displayed at 100 units to
        ///         the right and 100 units down from the top-left corner of this panel, exactly how <see cref="Canvas" /> would
        ///         display it.
        ///         If the value of <see cref="Scale" /> is set to <c>0.8</c> then the top-left corner of the element will be
        ///         displayed 80 units to the right and 80 units down from the top-left corner of this panel.
        ///         If <see cref="ApplyTransform" /> is set to <c>true</c> (the default value), then the element will also be
        ///         scaled down (shrunk) to 80% of its normal size, so that the bottom-right of the element will be 120 units to
        ///         the right and 120 units down from the top-left corner of this panel.
        ///         If <see cref="ApplyTransform" /> is set to <c>false</c>, then the element will remain its original size,
        ///         resulting in the bottom-right of the element being 130 units to the right and 130 units down from the top-left
        ///         corner of this panel.
        ///         In other words, it will simply have appeared to "move" up by 20 units and left by 20 units without changing its
        ///         size.
        ///         This is not normally what a user would expect when "zooming out" (unless the element is some kind of floating
        ///         label above the canvas), so it is expected that the children of the canvas will be responsible for changing
        ///         their representation appropriately when <see cref="ApplyTransform" /> is set to <c>false</c>.
        ///     </para>
        ///     <para>
        ///         When the <see cref="Viewbox" /> property is set to a non-<see cref="Rect.Empty" /> value, the value of the
        ///         <see cref="Scale" /> property will be automatically computed to match the <see cref="Viewbox" />,
        ///         <see cref="Stretch" />, and <see cref="StretchDirection" /> properties.
        ///         The value of the <see cref="Scale" /> property will contain the computed value (via the WPF dependency property
        ///         coersion mechanism), but and any attempts to set <see cref="Scale" /> to a different value will be ignored (if
        ///         <see cref="StretchDirection" /> is set to <see cref="StretchDirection.Both" />) until <see cref="Viewbox" /> is
        ///         set to <see cref="Rect.Empty" /> again.
        ///         <para>
        /// </remarks>
        /// <seealso cref="Viewbox" />
        public double Scale
        ...
        /// <summary>
        ///     Returns a <see cref="Double" /> representing the scale of the content that is currently being displayed after
        ///     taking <see cref="Viewbox" />, <see cref="Stretch" />, and <see cref="StretchDirection" /> into account.
        /// </summary>
        /// <param name="d">Dependency object whos value is being coerced.</param>
        /// <param name="value">The original uncoerced value.</param>
        ...
        public static readonly DependencyProperty RealizationLimitProperty =
            DependencyProperty.Register("RealizationLimit", typeof (int), typeof (ZoomableCanvas),
        ...
        /// </summary>
        /// <value>
        ///     An <see cref="Int32" /> between <c>0</c> and <see cref="Int32.MaxValue" />.  The default is
        ///     <see cref="Int32.MaxValue" />.
        /// </value>
        /// <remarks>
        ///     When the children of the canvas are being populated through an <see cref="ItemsControl" />, visual elements will be
        ///     instantiated for the first <see cref="RealizationLimit" /> items within the <see cref="ActualViewbox" />.
        /// </remarks>
        ...
        /// <summary>
        ///     Handles the event that occurs when the value of the <see cref="RealizationLimit" /> dependency property has
        ///     changed.
        /// </summary>
        /// <param name="d">The dependency object on which the dependency property has changed.</param>
        /// <param name="e">The event args containing the old and new values of the dependency property.</param>
        ...
        public static readonly DependencyProperty RealizationRateProperty =
            DependencyProperty.Register("RealizationRate", typeof (int), typeof (ZoomableCanvas),
                new FrameworkPropertyMetadata(int.MaxValue, OnRealizationRateChanged), IsRealizationRateValid);

        /// <summary>
        ///     Gets or sets the maximum number of elements that will be realized or virtualized at one time before yielding
        ///     control back to the dispatcher when <see cref="IsVirtualizing" /> is set to <see cref="true" />.
        /// </summary>
        /// <value>
        ///     An <see cref="Int32" /> between <c>0</c> and <see cref="Int32.MaxValue" />.  The default is
        ///     <see cref="Int32.MaxValue" />.
        /// </value>
        /// <remarks>
        ///     By default, the value of this property is <see cref="Int32.MaxValue" /> which means that all realization and
        ///     virtualization happens at once, at the time determined by <see cref="RealizationPriority" />.
        ///     The default behavior is optimized to realize all elements as quickly as possible, at the expensive of application
        ///     responsiveness while the realization is happening.
        ///     Setting the <see cref="RealizationPriority" /> to <see cref="DispatcherPriority.Input" /> and decreasing the
        ///     <see cref="RealizationRate" /> will make the application feel more responsive but will take longer to realize all
        ///     items.
        /// </remarks>
        ...
        {
            RenderTransformProperty.OverrideMetadata(typeof (ZoomableCanvas),
                new FrameworkPropertyMetadata(null, CoerceRenderTransform));

            try
            {
                Canvas.TopProperty.OverrideMetadata(typeof (UIElement),
                    new FrameworkPropertyMetadata(OnPositioningChanged));
                Canvas.LeftProperty.OverrideMetadata(typeof (UIElement),
                    new FrameworkPropertyMetadata(OnPositioningChanged));
                Canvas.BottomProperty.OverrideMetadata(typeof (UIElement),
                    new FrameworkPropertyMetadata(OnPositioningChanged));
                Canvas.RightProperty.OverrideMetadata(typeof (UIElement),
                    new FrameworkPropertyMetadata(OnPositioningChanged));
            }
        ...
        /// <summary>
        ///     Handles the event that occurs when the value of the <see cref="VirtualPanel.IsVirtualizing" /> property has
        ///     changed.
        /// </summary>
        ...
        /// <summary>
        ///     Provides a two-dimensional index of items that can be quickly queried for all items that intersect a given
        ///     rectangle.
        /// </summary>
        /// <remarks>
        ///     When the <see cref="ZoomableCanvas" /> is hosting items for an <see cref="ItemsControl" />, the
        ///     <see cref="ItemsControl.ItemsSource" /> can implement this interface to greatly speed up virtualization in the
        ///     canvas.
        ///     If any of those conditions are not true, then the canvas must realize every item at least once in order to
        ///     determine its bounds before it can virtualize it, and then once it is virtualized it will have no means of moving
        ///     spontaneously back into view.
        /// </remarks>
        [SuppressMessage("Microsoft.Design", "CA1034:NestedTypesShouldNotBeVisible",
            Justification = "This interface is only used by ZoomableCanvas.")]
        public interface ISpatialItemsSource
        {
            /// <summary>
            ///     Gets the entire extent of the index, which is typically the union of all bounding boxes of all items within the
            ///     set.
            /// </summary>
            /// <remarks>
            ...
            /// <remarks>
            ///     The enumerator returned by this method is used lazily and sometimes only partially, meaning it should return
            ///     quickly without computing the entire result set immediately for best results.
            /// </remarks>
        ...
        private class PrivateSpatialIndex : ISpatialItemsSource
        {
                ...
                set
                {
                    ...
                    if (bounds != value)
                    {
                        ...
                        if (QueryInvalidated != null &&
                            (bounds.IntersectsWith(_lastQuery) || value.IntersectsWith(_lastQuery)))
                        {
            ...
            /// <returns>
            ///     List of zero or more items that intersect the given bounds, returned in the order given by the priority assigned
            ///     during Insert.
            /// </returns>
            ...
            /// <summary>
            ///     Gets the computed minimum required rectangle to contain all of the items in the index.  This property is also
            ///     settable for efficiency the future extent of the items is known.
            /// </summary>
            public Rect Extent
            {
                get
        ...
        /// <summary>
        ///     Updates the calculated <see cref="ActualViewbox" /> and the <see cref="Scale" /> and <see cref="Offset" /> when the
        ///     size changes.
        /// </summary>
        ...
        /// <summary>
        ///     Scales the child elements of a <see cref="ZoomableCanvas" /> by applying a transform if
        ///     <see cref="ApplyTransform" /> is <c>true</c>, or by calling <see cref="FrameworkElement.InvalidateArrange" />
        ///     otherwise.
        /// </summary>
        ...
        /// <summary>
        ///     Measures the child elements of a <see cref="ZoomableCanvas" /> in anticipation of arranging them during the
        ...
        ///     The size that this <see cref="ZoomableCanvas" /> element should use to arrange its child
        ///     elements.
        /// </param>
        /// <returns>
        ///     A <see cref="Size" /> that represents the arranged size of this <see cref="ZoomableCanvas" /> element and its
        ///     descendants.
        /// </returns>
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            foreach (UIElement child in InternalChildren)
            {
                if (child != null)
                {
                    ...
                    var bounds = new Rect(Canvas.GetLeft(child).GetValueOrDefault(),
                        Canvas.GetTop(child).GetValueOrDefault(), child.DesiredSize.Width/scale,
        ...
        ///     <see cref="UIElement.ClipToBounds" /> property is set to <c>true</c>.
        /// </summary>
        /// <param name="layoutSlotSize">The available size of the element.</param>
        /// <returns>
        ///     A <see cref="Geometry" /> that represents the area that is clipped if <see cref="UIElement.ClipToBounds" /> is
        ///     <c>true</c>.
        /// </returns>
        ...
        /// <summary>
        ///     Caches the calculated <see cref="Extent" /> based on the spatial index and arranged children of the canvas until
        ...
        /// </summary>
        /// <remarks>
        ///     This property is also used to determine the range of the scroll bars when the canvas is hosted within a
        ...
        /// <returns>
        ///     The current position of the canvas point on the screen relative to the upper-left corner of this
        ///     <see cref="ZoomableCanvas" />.
        /// </returns>
        ...
        ///     The <see cref="Point" /> on the screen relative to the upper-left corner of this
        ///     <see cref="ZoomableCanvas" />.
        /// </param>
        ...
        {
            get { return ActualViewbox.Height*Scale; }
        }

        [SuppressMessage("Microsoft.Design", "CA1033:InterfaceMethodsShouldBeCallableByChildTypes")]
        double IScrollInfo.ViewportWidth
        {
            get { return ActualViewbox.Width*Scale; }
        }
        ...
        double IScrollInfo.ExtentHeight
        {
            ...
            {
                return
                    Math.Max(Math.Max(ActualViewbox.Bottom, Extent.Bottom) - Math.Min(ActualViewbox.Top, Extent.Top),
                        0.0)*Scale;
            }
        }
        ...
        double IScrollInfo.ExtentWidth
        {
            ...
            {
                return
                    Math.Max(Math.Max(ActualViewbox.Right, Extent.Right) - Math.Min(ActualViewbox.Left, Extent.Left),
                        0.0)*Scale;
            }
        }
        ...
        {
            get { return Math.Max(ActualViewbox.X - Extent.X, 0.0)*Scale; }
        }

        [SuppressMessage("Microsoft.Design", "CA1033:InterfaceMethodsShouldBeCallableByChildTypes")]
        double IScrollInfo.VerticalOffset
        {
            get { return Math.Max(ActualViewbox.Y - Extent.Y, 0.0)*Scale; }
        }
        ...
        {
            ((IScrollInfo) this).SetVerticalOffset(((IScrollInfo) this).VerticalOffset +
                                                   ((IScrollInfo) this).ViewportHeight);
        }

        [SuppressMessage("Microsoft.Design", "CA1033:InterfaceMethodsShouldBeCallableByChildTypes")]
        void IScrollInfo.PageLeft()
        {
            ((IScrollInfo) this).SetHorizontalOffset(((IScrollInfo) this).HorizontalOffset -
                                                     ((IScrollInfo) this).ViewportWidth);
        }
